...
Use before declaring a task complete. Produces the verification evidence required for the definition of done.
...
7. For UI changes: also check against the design review checklist (spacing grid, type scale, 5 states, depth rule, grayscale-first, no one-off values).
8. Record evidence in `ai/artifacts/<Epic>/verification/<card-id>.md`.
